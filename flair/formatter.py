...
def main():
    inputFile = open('unlabelled_test_data_for_annotation_combined_lower.csv')
    ...
    with open('flair-cleaned-testing-custom-annotation-all-lower.txt', 'w') as f:
        for row in csvreader:
            # text = row[0].replace(":", "")
            text = tokenize(row[0].replace(":", ""))
            outputLine = "__label__" + row[1] + " " + text + "\n"
            f.write(outputLine)
            rows.append(row)
